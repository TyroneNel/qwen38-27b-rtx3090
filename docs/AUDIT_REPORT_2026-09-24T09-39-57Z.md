...
Feedback loop: `bench/prepare_crash.py` (on B1). It runs the `docker/prepare.sh` sequence on a synthetic model, kills the process after each file write, runs one clean pass, and compares the result with a run without a kill. The run without a kill also checks that each backup is the file as it was before its step. A second scenario runs `quant_heads_stream.py` on a single-shard export.

Run it from a worktree in WSL: `cd /mnt/g/dev/hq-p3-<id> && /mnt/g/dev/qwen38-27b-rtx3090/venv/bin/python bench/prepare_crash.py [--steps ...]`.

Baseline on `upstream/main` `1cf8665` (all steps): 31 PASS, 36 FAIL, `RESULT FAIL`. The `stream` reference pass fails: `quant_heads_stream.py` loses the pristine `.bak-orig` on a single-shard export on every run, also without a kill.

| Id | Branch | Base | Scope | Status | Evidence (`1cf8665` → branch) |
|---|---|---|---|---|---|
| B1 | `fix/prepare-atomic-publish` | `upstream/main` | `prepare/atomic_publish.py`, both template writers, `bench/prepare_crash.py`, `prepare/README.md` | Committed `5e3ebb6` | `--steps harden,translate`: 4 PASS / 4 FAIL → 16 PASS / 0 FAIL |
| B2 | `fix/prepare-heads-commit-order` | B1 | `quant_lm_head.py`, `quant_embed.py`, `quant_mtp.py`: atomic writes, index last, resume from a packed shard, first backup kept | Committed `e9c70d6` | `--steps lm_head,embed,mtp`: 9 PASS / 23 FAIL → 50 PASS / 0 FAIL |
| B3 | `fix/prepare-heads-stream-publish` | B1 | `quant_heads_stream.py`: no missing-shard window, first `.bak-orig` kept, config before index, resume | Committed `9fd52b3` | `--steps stream`: reference FAIL → 22 PASS / 0 FAIL |
| B4 | `fix/prepare-draft-vocab-commit` | B1 | `build_draft_vocab.py`: atomic writes, ids before index (P-6) | Committed `44c36b5` | `--steps draft`: 4 PASS / 4 FAIL → 14 PASS / 0 FAIL |
| B5 | `fix/prepare-fast-variant-publish` | B1 | `fetch_fast_variant.py`: atomic copies, index last | Committed `85c4d6c` | `--steps fast`: 14 PASS / 4 FAIL → 34 PASS / 0 FAIL |
| B6 | `fix/prepare-state-torn-files` | `upstream/main` | `docker/prepare.sh` `state()`: unreadable index or config, or a truncated shard, returns `download`; `bench/test_prepare_state.py` in CI | Committed `461473a` | `bench/test_prepare_state.py`: 5 of 7 cases FAIL → OK |

Integration (detached `../hq-p3-int` = B1 + merges of B2 to B6): `bench/prepare_crash.py` all steps 136 PASS / 0 FAIL, `RESULT PASS` (57 writes in `prepare`, 11 in `stream`). `bench/test_prepare_state.py` OK. `bench/test_model_verification.py` OK. `bash -n docker/prepare.sh` OK.
...
- The upstream CI installs no torch. `bench/prepare_crash.py` runs by hand only. B6 adds the torch-free `bench/test_prepare_state.py` to CI. A CI job with CPU torch for the crash harness is a possible later PR.
...
- No run on a real 19.5 GB checkpoint. The harness runs the real scripts on a synthetic model with the same layout.

### 8.4 PR drafts

Each PR body follows the `pr` template and ASD-STE100 structure. Evidence lines come from 8.3.

**B1: prepare: write the chat templates atomically, add a crash-injection harness (#195)**
...
bench/prepare_crash.py                  # new: kill after each write, then one clean pass
```

Evidence: `--steps harden,translate`: before 4 PASS / 4 FAIL, after 16 PASS / 0 FAIL. Door: two-way. Blast radius: template writers only; the helper is new code with no other caller yet.
...
Evidence: `--steps lm_head,embed,mtp`: before 9 PASS / 23 FAIL (KeyError, JSONDecodeError), after 50 PASS / 0 FAIL. Door: two-way. Blast radius: first-start preparation; output bytes are unchanged (the harness compares them with a run without a kill).

**B3: prepare: keep quant_heads_stream.py's live shard and first .bak-orig, publish the index last (#195)**

Evidence: `--steps stream`: before, the reference pass fails (`.bak-orig` holds the modified shard); after, 22 PASS / 0 FAIL. Door: two-way. Blast radius: third-party checkpoint preparation only.

**B4: prepare: write the draft vocabulary atomically, ids before the index (#195)**

Evidence: `--steps draft`: before 4 PASS / 4 FAIL, after 14 PASS / 0 FAIL. Door: two-way. Blast radius: the draft-vocab step.

**B5: prepare: copy the fast variant's files atomically, index last (#195)**

Evidence: `--steps fast`: before 14 PASS / 4 FAIL, after 34 PASS / 0 FAIL. Door: two-way. Blast radius: the `-fast` directory assembly.
