...
- 2026-09-25: Tier 1 on upstream. Dev venv rebuilt on the upstream pins (`venv-0.28.0.bak` kept); series applied at `--fuzz 0`, KVarN installed; `verify.sh --install` has 1 FAIL (FlashInfer is not in `docker/requirements.txt`). P-1 reproduced live and filed as #204. P-2 not reachable on the served model. P-4 fork only. Local `-fast` dir: shard 6 is `root` mode 0600, so the WSL user cannot read it (a permission problem, not a code defect).
