...
|---|---|---|
| P-1 | Applies, reproduced | Live on the 3090 (`1cf8665`, vllm 0.29.0, no key): `ss` shows `0.0.0.0:18020`; chat, `/v1/models`, `/tokenize`, `/metrics`, `/docs` answer 200 with no key from the WSL2 interface address and from the Windows host. Also `alternative.sh:32` exports an empty key when `api_key.txt` is missing. Upstream made the key optional on purpose (`999e264`), so the issue proposes a loopback default when no key is set, not a refusal. Filed [#204](https://github.com/syv-ai/HyperQwen/issues/204). |
| P-2 | Not reachable, filed as LOW | The `zip()` mismatch needs a group with `prefix_cacheable=False`. In the patched 0.29.0 tree only `CircularBufferSpec` returns `False`, and only `models/qwen4_exp` uses it; no patch or KVarN file overrides the property. Runtime check in the patched venv (Qwen3.5, MTP, DFlash2, KVarN loaded): 14 spec classes, only `CircularBufferSpec` not cacheable. The extra `hash % block_size == 0` condition is dead: `hash_block_size` is the GCD of the cacheable block sizes, and `resolve_kv_cache_block_sizes` raises for a non-divisible `prefix_match_unit`. Filed as a low-priority code-quality note: [#206](https://github.com/syv-ai/HyperQwen/issues/206). |
| P-3 | Applies, worse | All writers write in place upstream. Tracked in 8.3. |
| P-4 | Fork only, **closed** | `bench/paired_run.sh` does not exist upstream, so it was fixed on the fork: `export PORT` after argument parsing, so the measure step targets the booted `--port`. Regression test `bench/test_paired_run_port.py` (no GPU; run in `bench-policy.yml`): a booted stub on `--port` and a decoy on the measure script's default port. Before the fix, all 4 repeats hit the decoy; after, 4 hit the booted server and 0 the decoy. The audit's second step (reject a health answer older than the start) was not added: `_boot` already refuses a port that is healthy before the boot, so the only server on `--port` is the one the driver started. |
...
Pushed to `origin` and opened against `syv-ai/HyperQwen:main` on 2026-09-25: B1 [#198](https://github.com/syv-ai/HyperQwen/pull/198), B2 [#199](https://github.com/syv-ai/HyperQwen/pull/199), B3 [#200](https://github.com/syv-ai/HyperQwen/pull/200), B4 [#201](https://github.com/syv-ai/HyperQwen/pull/201), B5 [#202](https://github.com/syv-ai/HyperQwen/pull/202), B6 [#203](https://github.com/syv-ai/HyperQwen/pull/203). Each PR names #195; B2 to B5 state their dependency on #198 and need a rebase after #198 merges. PR bodies are in `../hq-p3-pr/`.
...
- 2026-09-25: Tier 1 on upstream. Dev venv rebuilt on the upstream pins (`venv-0.28.0.bak` kept); series applied at `--fuzz 0`, KVarN installed; `verify.sh --install` has 1 FAIL (FlashInfer: no cubin and no nvcc in the venv; fixed by installing `flashinfer-cubin==0.6.18` from `https://flashinfer.ai/whl/`). Correction: this entry first said the cause was that FlashInfer is not in `docker/requirements.txt`. That omission is deliberate upstream (`docs/install.md`: the image has nvcc; the cubin is 6.3 GB). P-1 reproduced live and filed as #204. P-2 not reachable on the served model. P-4 fork only. Local `-fast` dir: shard 6 is `root` mode 0600, so the WSL user cannot read it (a permission problem, not a code defect).
- 2026-09-25: P-4 fixed on the fork (`48fd1d7`). Opened [#207](https://github.com/syv-ai/HyperQwen/pull/207) (branch `fix/verify-flashinfer-cubin-hint`, off `upstream/main` `1cf8665`): the `verify.sh` FlashInfer hint said `flashinfer-cubin==0.6.13` against the 0.29.0 pin's `flashinfer-python==0.6.18`; it now names the cubin for the installed `flashinfer-python` from `flashinfer.ai/whl`. Stale "0.6.13 against 0.6.16.post3" comments corrected in both launchers. No overlap with #189's hunks. `docs/python-314.md:65` (a 0.27.1-stack page) still says 0.6.13; left alone.
