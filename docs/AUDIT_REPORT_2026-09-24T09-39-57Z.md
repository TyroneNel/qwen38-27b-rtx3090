...
| P-2 | Not reachable, not filed | The `zip()` mismatch needs a group with `prefix_cacheable=False`. In the patched 0.29.0 tree only `CircularBufferSpec` returns `False`, and only `models/qwen4_exp` uses it; no patch or KVarN file overrides the property. The extra `hash % block_size == 0` condition is dead: `hash_block_size` is the GCD of the cacheable block sizes, and `resolve_kv_cache_block_sizes` raises for a non-divisible `prefix_match_unit`. Code hygiene only. |
